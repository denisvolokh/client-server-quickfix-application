...
class AcceptorApplication(Application):
    ...
    def __init__(self, logger):
        super(AcceptorApplication, self).__init__()

        self.logger = logger
        self.trades_store = Store()
        self.securities_store = Store()

    def onCreate(self, session_id):
        self.session_id = session_id
        self.logger.info("[+] On Create: {}".format(session_id))
        return
    ...
    def onLogout(self, session_id):
        self.logger.info("[+] On Logout: %s" % session_id)
        return

    def toApp(self, message, session_id):
        self.logger.info("[+] On toApp: %s" % message)
        return

    def fromApp(self, message, session_id):
        self.logger.info("[+] On fromApp: %s" % message)
        ...
        if message_type_value == fix.MsgType(fix.MsgType_SecurityDefinitionRequest).getValue():
            self.logger.info("   [+] Received Security Definition Request!")
        ...
        if message_type_value == fix.MsgType(fix.MsgType_TradeCaptureReportRequest).getValue():
            self.logger.info("   [+] Received Trade Capture Report Request!")
        ...
        if message_type_value == fix.MsgType(fix.MsgType_MarketDataRequest).getValue():
            self.logger.info("   [+] Received Market Data Request!")
    ...
    def toAdmin(self, message, session_id):
        self.logger.info("[+] On toAdmin: %s" % message)
        return

    def fromAdmin(self, message, session_id):
        self.logger.info("[+] On fromAdmin: %s" % message)
    ...
    def _process_market_data_request(self, message, session_id):
        ...
        if "REJECT" in md_req_id_value.upper():
            ...
            self.logger.info("   [+] Sending Reject message!")

            try:
                fix.Session.sendToTarget(reject_message, session_id)

            except Exception:
                self.logger.info("[!!!] Session Not found!")
    ...
    def _ack_trade_capture_request(self, message, session_id):
        self.logger.info("  [+] Ack trade capture request ...")
        ...
        self.logger.info("   [+] Subscription Type: {}".format(SUBSCRIPTION_TYPES.get(subscription_type_value)))
        ...
        self.logger.info("   [+] Sending Ack message!")
    ...
    def send_report(self):
        self._send_trade_capture_report(session_id=self.session_id)

    def _send_trade_capture_report(self, session_id):
        self.logger.info("  [+] Send trade capture report ...")

        trades = self.trades_store.get("trades")

        if trades:
            self.logger.info("   [+] Trades retrieved from cache")

        if not trades:
            response = requests.get(url="http://restapi:5555/trades")
            self.logger.info("   [+] Trades API response: {0}".format(response.json()))
            trades = response.json()
            self.trades_store.put(key="trades", value=trades)
        ...
        for index, trade_item in enumerate(trades):
            ...
            report_mesage.setField(fix.ApplID(str(index)))
            ...
            report_mesage.setField(fix.TransactTime())
    ...
    def _process_security_definition_request(self, message, session_id):
        ...
        self.logger.info("  [+] Security Request Id: {}".format(securty_request_id_value))

        if securty_type_value == fix.SecurityType_CORPORATE_BOND:

            bonds = self.securities_store.get("bonds")

            if bonds:
                self.logger.info("   [+] Bonds retrieved from cache")

            if not bonds:
                response = requests.get(url="http://restapi:5555/bonds")
                self.logger.info("   [+] Bonds API response: {0}".format(response.json()))
                bonds = response.json()
                self.securities_store.put(key="bonds", value=bonds)
    ...
    def _send_security_definition(self, security_req_id, data, session_id):
        self.logger.info("   [+] Sending definition: {}".format(data["internalcode"]))
